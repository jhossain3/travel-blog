...
import { Grid, Typography, Card, CardActionArea} from "@mui/material";
          ...
          return (
            <Grid key={i} item>
              <CardActionArea component={Link} to={`/posts/${posts[i]._id}`} >
                ...
                >
                  <Typography color='#FFFFFF' variant="h4" sx={{fontWeight: 'bold', mb:1, ml:1}}>{posts[i].destination}</Typography>
                  <Typography color='#FFFFFF' sx={{mb:1, ml:1}}> {posts[i].author.name}</Typography>
                  <Grid sx={{color:'#FFFFFF', mb:'2',mt:'5'}}>
                  <Button component={Link} to={`/posts/${posts[i]._id}/update`} color="primary" size="large" startIcon={<EditIcon />}>
                  </Button>
